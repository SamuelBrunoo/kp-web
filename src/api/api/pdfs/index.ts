...
import { TApi_Params_PDF as TParams } from "./params"
import { TApi_Responses_PDF as TResponses } from "./responses"
    ...
    try {
      await service
        ...
        .then((res) => {
          ...
          let filename = `Pedido${filters.forAdmin ? " completo" : ""}.pdf`
...
export const getSlipPdf: TApi["pdfs"]["getSlipPdf"] = async (info) => {
  return new Promise(async (resolve, reject) => {
    try {
      await service
        .get(`${baseURL}/slip`, {
          params: {
            slipCode: info.slipCode,
          },
          responseType: "blob",
        })
        .then((res) => {
          ...
          const disposition = res.headers["Content-Disposition"]
          let filename = `Boleto [${info.clientName}] ${info.slipInstallment} de ${info.totalInstallments}.pdf`
...
export type TApi_Pdfs = {
  ...
  getSlipPdf: (
    p: TParams["pdfs"]["getSlipPdf"]
  ) => TResponses["pdfs"]["getSlipPdf"]
}
